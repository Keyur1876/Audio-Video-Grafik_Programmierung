...
bool CMCIObject::OpenFile(LPCWSTR pszFileName) { //OpenFile MCI7
	if (m_op.wDeviceID != 0) {
		Close();
	}
	m_op.lpstrDeviceType = 0;
...
bool CMCIObject::OpenAudioCD(LPCWSTR drive, BYTE& tracks) { //OpenDevice MCI7
	DWORD flag = 0;
...
void CMCIObject::MCIError()
{
	...
	if (!strlen(buf))
		...
		//Strcpy Error (Sicherheit?) _CRT_SECURE_NO_WARNINGS
	AfxMessageBox((LPCWSTR)buf);
	m_op.wDeviceID = 0;
}

bool CMCIObject::SetVideoPosition(HWND hwnd, CRect rect) { //SetAviPosition
